...
2. **Фото** (необязательно):
   - `images/login.jpg` — фото внизу страницы с паролем
   - `images/letter.jpg` — картинка формата A4 на второй странице
3. **Шрифт** — Inter Medium подключён через Google Fonts в `<head>` (строка с `fonts.googleapis.com`)
